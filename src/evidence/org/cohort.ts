...
/**
 * The cohort, or an EMPTY ONE where none has been collected yet.
 *
 * The serving path must not fail on an uncollected graph, and this is the difference between the two callers.
 * `collect` refusing is right: it is about to spend an hour's quota deciding what to fetch, and "no graph" means
 * it cannot know. A dashboard page has no such stake — it should render an estate of nothing with the collection
 * notice explaining why, which is the same "absent means nobody measured it" contract the rest of the report
 * follows.
 *
 * Found the hard way: a preview environment deploys with `orgJob.enabled: false` and an empty database, so
 * `/repositories`, `/teams` and `/contributors` all threw and `@smoke` — whose whole job is to assert that pages
 * render on exactly that database — failed on all three.
 */
export async function servedCohort(configuration: Configuration, reference = new Date()): Promise<CohortEntry[]> {
  try {
    return await readCohort(configuration, reference);
  } catch (error) {
    if (error instanceof CohortUncollectedError) {
      console.warn(`serving an empty estate: ${error.message}`);
      return [];
    }
    throw error;
  }
