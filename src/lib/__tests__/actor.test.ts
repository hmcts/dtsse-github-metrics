...
function row(repository: string, contributions: number, metrics: ActorRepositoryReadiness["metrics"] = []): ActorRepositoryReadiness {
...
describe("activity", () => {
  ...
  it("pluralises each count against its own noun", () => {
    expect(activity({ actor_login: "bob", repositories: [row("api", 1)] })).toBe("1 merge across 1 repository");
  });
