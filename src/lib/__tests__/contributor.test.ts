...
function coverage(numerator: number, denominator: number, classifications: Record<string, number> = {}): BehaviourMetricSummary {
  return {
...
describe("contributorFigures", () => {
  ...
  it("separates the two routes for a mixed window, and the unreviewed merges from both", () => {
    const figures = contributorFigures(row([coverage(5, 9, { included: 5, "no-review-events": 2, "direct-commit": 2 }), size(48.5)]));
