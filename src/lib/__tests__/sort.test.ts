...
import { compare, nextDirection, reverse, type SortValue, sorted } from "@/lib/sort";
...
const rows: Row[] = [{ repository: "delta", open: 2 }, { repository: "alpha", open: 7 }, { repository: "charlie" }, { repository: "bravo", open: 2 }];
...
describe("sorted", () => {
  it("orders by the read value ascending", () => {
    expect(names(sorted(rows, (row) => row.repository, "ascending"))).toEqual(["alpha", "bravo", "charlie", "delta"]);
  });

  it("orders descending without disturbing ties", () => {
    expect(names(sorted(rows, (row) => row.open, "descending"))).toEqual(["alpha", "delta", "bravo", "charlie"]);
  });
