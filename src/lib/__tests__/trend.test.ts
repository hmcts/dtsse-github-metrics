...
describe("measures", () => {
  ...
  it("collects the measures across every window, not from whichever one comes first", () => {
    ...
    expect(seriesMeasures(suppressed).map((metric) => metric.metric)).toEqual(["approval-coverage", "merge-cycle-time"]);
  });
});
...
describe("metricCharts", () => {
  const charts = metricCharts(series({ periods: [period(1, { metrics: [RATE] }), period(2)] }));
...
describe("windowReasons", () => {
  it("names the window each reason belongs to, under the charts rather than on them", () => {
    ...
    expect(windowReasons(suppressed)).toEqual(["Baseline (2026-06-01): 3 merges, below the minimum of 10"]);
  });
