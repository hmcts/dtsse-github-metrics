...
describe("conditionGroups", () => {
  ...
  it("returns all three groups so a green is as auditable as a red", () => {
    expect(conditionGroups(assessment).map((group) => group.key)).toEqual(["blocking", "caution", "clear"]);
  });
  ...
  it("carries the policy’s own conditions through untouched", () => {
    expect(conditionGroups(assessment)[1]?.conditions[0]?.detail).toBe("the gate requires no status check");
  });
  ...
  it("leaves the caution group in the policy’s order, informational rows included", () => {
    ...
    expect(conditionGroups(mixed)[1]?.conditions.map((condition) => condition.condition)).toEqual(["reported", "no required check"]);
  });
});
...
describe("mergeGateRows", () => {
  ...
  it("keeps an administrator answer GitHub withheld out of the false one", () => {
    ...
    expect(value(mergeGateRows(gate({ applies_to_administrators: false })), "Applies to administrators")).toBe("no");
  });
  ...
  it("reads a gate that requires review and checks as satisfying both", () => {
    const rows = mergeGateRows(
      gate({
        ...
        status_checks: [{ strict_required_status_checks_policy: true, required_status_checks: [{ context: "build" }] }],
...
describe("security alerts", () => {
  ...
  it("counts an asserted severity and zero for one GitHub asserted nothing about", () => {
    expect(severityDetail("dependabot", alerts.dependabot)).toBe("critical 1 · high 2 · medium 0 · low 0");
  });
...
describe("maintenance", () => {
  ...
  it("keeps an unsearched human answer unknown, with the reason beside it", () => {
    expect(maintenanceRows(report)[1]?.detail).toBe("human commit: unknown — the search stopped at 2026-02-01");
  });
  ...
  it("weighs a window with no commit in it rather than condemning the repository", () => {
    const rows = maintenanceRows({
      ...report,
      windows: [
        { months: 3, committed_within: false },
        { months: 12, committed_within: true }
      ]
    });
...
describe("SonarCloud", () => {
  ...
  it("colours a passing gate green and a project with no gate configured not at all", () => {
    expect(sonarGateCard({ measures: { ...measures, gate: { level: "OK", conditions: [] } } }).tone).toBe("good");
    // NONE is Sonar's word for a project with no gate conditions, which is an absence not a pass.
    expect(sonarGateCard({ measures: { ...measures, gate: { level: "NONE", conditions: [] } } }).tone).toBe("neutral");
  });
