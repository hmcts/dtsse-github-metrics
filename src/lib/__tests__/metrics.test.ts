...
describe("metricTone", () => {
  ...
  function assessment(blocking: ReadinessCondition[], caution: ReadinessCondition[], clear: ReadinessCondition[]): ReadinessAssessment {
  ...
  it("grades nothing off a condition whose ceiling could not be read", () => {
    const unreadable = assessment([{ condition: "approval-coverage-below-target", label: "cannot_assess", detail: "half the question" }], [], []);
  ...
  it("carries an informational condition without colour, as the assessment block does", () => {
    const reported = assessment([], [], [{ condition: "approval-coverage-at-target", detail: "reported, not judged", informational: true }]);
