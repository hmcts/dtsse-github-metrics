...
const ASSESSMENT: ReadinessAssessment = {
  label: "red",
  blocking: [{ condition: "merge gate", label: "red", detail: "the default branch is unprotected" }],
  caution: [],
...
describe("MetricsGrid", () => {
  ...
  it("keeps the report’s order rather than ordering by any figure", () => {
    const markup = renderToStaticMarkup(createElement(MetricsGrid, { summaries, empty: "none" }));
    expect(markup.indexOf("independent-review-coverage")).toBeLessThan(markup.indexOf("merge-cycle-time"));
  });
  ...
  it("colours a card from the condition that graded that metric", () => {
    ...
    const markup = renderToStaticMarkup(createElement(MetricsGrid, { summaries, empty: "none", assessment: graded }));
  ...
  it("says why there are no metrics rather than drawing an empty grid", () => {
    const markup = renderToStaticMarkup(createElement(MetricsGrid, { summaries: [], empty: "No metric was computed at this span." }));
  ...
  const findings: PracticeFinding[] = [
    {
      ...
      pull_requests: [{ number: 41, url: "https://github.com/hmcts/api/pull/41", merged_at: "2026-08-02T00:00:00Z", size_class: "small" }],
      direct_commits: [
        {
...
describe("ContributorsTable", () => {
  ...
  function contributor(login: string, contributions: number, numerator: number, denominator: number, classifications: Record<string, number>) {
    return {
      login,
      contributions,
      blocking: 0,
  ...
  const markup = renderToStaticMarkup(
    createElement(ContributorsTable, {
      ...
      rows: [contributor("carol", 9, 4, 9, { included: 4, "no-review-events": 2, "direct-commit": 3 }), contributor("alice", 3, 3, 3, { included: 3 })],
      weeks: 4
    })
  );
