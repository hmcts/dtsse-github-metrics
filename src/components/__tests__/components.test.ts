...
describe("MetricCard", () => {
  it("states the figure, its name, and what it was measured over", () => {
    const markup = renderToStaticMarkup(createElement(MetricCard, { label: "Merged", value: "42", detail: "42 of 50" }));
  ...
  it("colours the value it was given a tone for, and nothing else on the card", () => {
    const markup = renderToStaticMarkup(createElement(MetricCard, { label: "Direct commits", value: 214, detail: "of 300 merges", tone: "bad" }));
...
describe("Section", () => {
  ...
  it("bounds the whole section in one panel, heading included", () => {
    const markup = renderToStaticMarkup(createElement(Section, { heading: "Merge gate" }, createElement("p", null, "rows")));
    ...
    expect(markup).toMatch(/^<section class="bg-slate-900\/40 border border-slate-800 rounded-lg">.*Merge gate/);
  ...
  it("omits the detail and control rows when there are none", () => {
    const markup = renderToStaticMarkup(createElement(Section, { heading: "Findings" }, createElement("p", null, "rows")));
...
describe("DefinitionList", () => {
  ...
  it("sets a count in tabular figures and an answer in words in neither", () => {
    const counted = renderToStaticMarkup(createElement(DefinitionList, { values: [{ label: "Approving reviews required", value: "2" }] }));
    const worded = renderToStaticMarkup(createElement(DefinitionList, { values: [{ label: "Protected", value: "not disclosed" }] }));
    // The dash included: a column of counts with one absent value keeps its digits aligned.
    const absent = renderToStaticMarkup(createElement(DefinitionList, { values: [{ label: "code-scanning", value: "-" }] }));
  ...
  it("reads each tone through the one map, so a row cannot invent a colour", () => {
    for (const tone of TONES) {
      const markup = renderToStaticMarkup(createElement(DefinitionList, { values: [{ label: "Coverage", value: "82%", tone }] }));
      expect(markup).toContain(TONE_VALUE[tone]);
    }
  });
});
...
describe("EntityHeader", () => {
  ...
  it("grades nothing for a contributor or a team, and shows no trail of how you arrived", () => {
    const markup = renderToStaticMarkup(createElement(EntityHeader, { kind: "contributor", name: "octocat" }));
...
describe("Navigation", () => {
  ...
  it("lists the three estate links in estate order", () => {
    ...
    const order = ["Repositories", "Teams", "Contributors"].map((label) => markup.indexOf(`>${label}</a>`));
...
describe("InfoTooltip", () => {
  it("carries its text on the trigger, not in a native title attribute", () => {
    const markup = renderToStaticMarkup(createElement(InfoTooltip, { text: "Counted over merged pull requests only." }));
...
describe("WeekSpanButtons", () => {
  ...
  it("presses the span the page is at, and claims nothing is loading", () => {
    ...
    expect(markup).toContain('aria-label="4 week window" class="px-3 py-1.5 rounded text-sm transition-colors bg-indigo-600');
  });
  ...
  it("differs from the settled group in the busy state alone", () => {
    ...
    const restored = dimmed.replace(" opacity-50", "").replace('aria-busy="true"', 'aria-busy="false"');
...
describe("the shared vocabulary", () => {
  it("contains no emoji: a grade is a word and a colour, never a coloured square", () => {
    const markup = [
      ...
      renderToStaticMarkup(createElement(RAGRow, { label: "amber", condition: "sonar", detail: "gate errored" })),
      ...
      renderToStaticMarkup(createElement(DefinitionList, { values: [{ label: "Protected", value: "yes", tone: "good" }] })),
  ...
  function naming(): string {
    return [
      ...
      renderToStaticMarkup(createElement(FindingsTable, { findings: [FINDING], weeks: 4 })),
      renderToStaticMarkup(
