...
async function drawn(data: readonly PieSlice[] = SLICES, parameter?: string): Promise<HTMLElement> {
  const { container } = render(<SummaryPieChart title="Readiness" data={data} parameter={parameter} />);
  await act(async () => {
...
describe("SummaryPieChart hover card", () => {
  ...
  it("colours the slice name to match the wedge it came from", async () => {
    ...
    const name = within(container.querySelector(".recharts-tooltip-wrapper") as HTMLElement).getByText("Blocked");
